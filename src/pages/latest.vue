  ...
  <ion-page>
     <ion-header>
      <ion-toolbar>
        ...
        <ion-title>Latest threads</ion-title>
...
import {getAlerts, getForum, getLatestThreads} from '../utils/api'
...
const loadLatestThreads = async () => {
  try {
    ...
    const fetchedItems = await getLatestThreads()
    items.value = fetchedItems
    isFetching.value = false
  } catch (e) {
    const toast = await toastController
    .create({
      message: 'Failed to get latest threads.',
      color: 'danger',
      duration: 2000
    })
